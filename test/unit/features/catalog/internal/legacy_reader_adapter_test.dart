...
void main() {
  test('dispatches dictionary reads by CatalogId using the raw wordId',
    ...
    final reader = LegacyCatalogReaderAdapter(
  ...
  test('preserves the identical legacy failure instance', () async {
    final error = BusinessRuleError(message: 'legacy failure');
    final reader = LegacyCatalogReaderAdapter(
      espJpnRepository: _EspRepository(Result.failure(error)),
      jpnEspRepository: _JpnRepository(Result.success(const [])),
    );
  ...
  test('EspJpn conjugation uses raw wordId and preserves failure identity',
      () async {
    ...
    final reader = LegacyConjugationReaderAdapter(repository);
  ...
  test(
      'JpnEsp conjugation operations fail as unsupported without repository calls',
      () async {
    final repository = _ConjugationRepository(
...
final class _ConjugationRepository implements IConjugacionsRepository {
  _ConjugationRepository(
      {required this.conjugation, required this.hasConjugation});
