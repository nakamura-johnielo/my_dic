...
final class QuizCandidateEnrichment {
  QuizCandidateEnrichment(
    this._conjugations,
    this._dictionary,
    this._rankingLoader,
  );

  factory QuizCandidateEnrichment.withDatabase(
  ...
  Future<QuizCandidateEnrichmentData> load(
    List<int> wordIds,
    List<QuizCandidateIssue> issues,
  ) async {
    if (wordIds.isEmpty) return const QuizCandidateEnrichmentData();
    ...
    return QuizCandidateEnrichmentData(
  ...
  Future<Map<int, String>> _loadMeanings(List<int> wordIds) async {
    ...
    for (final entry in results[1].entries) {
      meanings.putIfAbsent(
          entry.key, () => extractQuizMeaningText(entry.value));
    }
  ...
  Future<Map<int, int>> _loadStars(List<int> wordIds) async {
    ...
    return headwords.map(
      (wordId, headword) =>
          MapEntry(wordId, quizStarCountFromHeadword(headword)),
    );
  }
...
final class QuizCandidateEnrichmentData {
  const QuizCandidateEnrichmentData({
    this.rankings = const {},
    this.meanings = const {},
