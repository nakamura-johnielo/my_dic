...
class UpdateMyWordInteractor implements IUpdateMyWordUseCase {
  ...
  UpdateMyWordInteractor(this._driftMyWordRepository, this._authRepository);
    ...
    try {
      ...
      authResult.when(
        ...
        failure: (error) => AppLogger.print(
          'Auth lookup failed during MyWord update: ${error.message}',
        ),
      );
    } catch (error) {
      AppLogger.print(
        'Unexpected auth lookup failure during MyWord update: $error',
      );
    }

    UpdateMyWordRepositoryInputData repositoryInput =
        UpdateMyWordRepositoryInputData(input.myWordId, input.headword.trim(),
