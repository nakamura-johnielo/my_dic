...
class DriftMyWordRepository implements IMyWordRepository {
  ...
  Future<Result<MyWord>> getById(int id) async {
    try {
      final res = await _myWordDao.getMyWordById(id);
      if (res == null) {
        return Result.failure(NotFoundError(
          message: '指定された単語が見つかりません',
        ));
      }
      return Result.success(MyWord(
        wordId: res.myWordId,
        word: res.word,
  ...
  Future<Result<int>> registerWord(
    ...
    try {
      ...
      final wordId = await _myWordDao.insertMyWord(
  ...
  Future<Result<void>> deleteWord(DeleteMyWordRepositoryInputData input) async {
    try {
      final affectedRows = await _myWordDao.deleteMyword(input.id, input.dateTime);
      if (affectedRows == 0) {
        return Result.failure(NotFoundError(
  ...
  Future<Result<void>> updateWord(UpdateMyWordRepositoryInputData input) async {
    try {
      final affectedRows = await _myWordDao.updateMyWord(
