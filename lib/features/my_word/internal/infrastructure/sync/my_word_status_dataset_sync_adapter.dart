...
final class MyWordStatusDatasetSyncAdapter implements DatasetSyncAdapter {
  MyWordStatusDatasetSyncAdapter({required IMyWordStatusLocalDataSource local, required IMyWordStatusRemoteDataSource remote})
      : _local = local,
        _remote = remote;
  ...
  @override
  Future<RemoteMutationAck> push(RemoteMutationRequest request) => _remote.patchStatus(request);
  @override
  Future<List<DatasetSyncRecord>> pull(String accountId, SyncCursor? cursor) async {
    ...
    return [for (final item in items) DatasetSyncRecord(entityId: item.myWordId, updatedAt: item.updatedAt, remoteRevision: item.remoteRevision, lastMutationId: item.lastMutationId, payload: item)];
  }
  @override
  Future<T> transaction<T>(Future<T> Function() action) => _local.runInTransaction(action);
  @override
  Future<bool> acknowledge({required SyncMutation mutation, required int leasedLocalRevision, required String accountId, required RemoteMutationAck acknowledgement}) => _local.acknowledgeRemoteMutation(myWordId: mutation.entityId, accountId: accountId, localRevision: leasedLocalRevision, remoteRevision: acknowledgement.remoteRevision.toString(), lastMutationId: acknowledgement.lastMutationId);
  @override
  Future<void> applyRemote(DatasetSyncRecord record, {required String accountId, required Set<String> skippedFields}) {
    ...
    return _local.applyRemoteFields(record.entityId,
        ...
        isBookmarked: skippedFields.contains('isBookmarked') ? null : item.isBookmarked,
        editAt: item.updatedAt.toIso8601String(), accountId: accountId,
        remoteRevision: record.remoteRevision.toString(), lastMutationId: record.lastMutationId);
  }
  DateTime _toDateTime(SyncCursor cursor) => DateTime.fromMicrosecondsSinceEpoch(cursor.seconds * 1000000 + cursor.nanoseconds ~/ 1000, isUtc: true);
}
