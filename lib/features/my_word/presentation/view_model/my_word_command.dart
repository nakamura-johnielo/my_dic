...
class MyWordRegistrationCommand extends _MyWordCommandBase {
  MyWordRegistrationCommand(this._registerUseCase);
...
class MyWordCommand extends _MyWordCommandBase {
  MyWordCommand(this._wordId, this._updateUseCase, this._deleteUseCase);
  ...
  Future<void> deleteWord() async {
    ...
    begin(operation);
    final result = await _deleteUseCase.execute(DeleteMyWordInputData(_wordId));
    result.when(
      success: (_) => succeed(operation),
      failure: (error) => fail(operation, error),
    );
