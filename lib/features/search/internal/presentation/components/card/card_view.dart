...
class ConjSections extends StatelessWidget {
  const ConjSections({super.key, required this.conjugacions, required this.query});
  final Map<SearchConjugationMatchKey, String> conjugacions;
  final String query;
  @override
  Widget build(BuildContext context) {
    final sections = conjugacions.entries.where((entry) => entry.value.isNotEmpty).map((entry) => _ConjMiniSection(keyValue: entry.key, conjugation: entry.value, query: query)).toList()
      ..sort((a, b) => a.conjugation == query ? -1 : (b.conjugation == query ? 1 : 0));
    return ClipRect(child: SingleChildScrollView(scrollDirection: Axis.horizontal, physics: const NeverScrollableScrollPhysics(), child: Row(mainAxisSize: MainAxisSize.min, spacing: 8, children: sections)));
  }
}

class _ConjMiniSection extends StatelessWidget {
  const _ConjMiniSection({required this.keyValue, required this.conjugation, required this.query});
  ...
  Widget build(BuildContext context) {
    ...
    if (keyValue.moodTense == SearchMoodTense.participlePast || keyValue.moodTense == SearchMoodTense.participlePresent) title = '${keyValue.moodTense.shortLabel}:';
    return Row(spacing: 3, children: [Text(title, style: const TextStyle(fontSize: 13)), _highlight(context)]);
  }
  Widget _highlight(BuildContext context) {
    final start = conjugation.toLowerCase().indexOf(query.toLowerCase());
    if (query.isEmpty || start < 0) return Text(conjugation, style: const TextStyle(fontSize: 15));
    final end = start + query.length;
    final exact = conjugation.length == query.length;
    return RichText(text: TextSpan(style: const TextStyle(fontSize: 15), children: [
      TextSpan(text: conjugation.substring(0, start)),
      TextSpan(text: conjugation.substring(start, end), style: TextStyle(fontWeight: FontWeight.bold, backgroundColor: exact ? Theme.of(context).colorScheme.primary : Theme.of(context).colorScheme.primary.withValues(alpha: .5), color: exact ? Theme.of(context).colorScheme.onPrimary : null)),
