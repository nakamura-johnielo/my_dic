...
スペイン語は動詞の活用が多く、文章中で見つけた語形から原形へたどり着くこと自体が学習の障壁になる。日本語・スペイン語・活用形のいずれからでも検索できる辞書に、フラッシュカード、学習状態管理、複数端末同期を組み合わせ、学習障壁を下げる。
...
- Unit・Widget・Integration・SecurityテストとGitHub Actionsによる継続的な品質確認

## Screenshots

辞書データの公開範囲を制限しているため、現在はスクリーンショットを中心に紹介しています。
...
本プロジェクトは、実際に欲しい語学学習体験を題材に、機能実装だけでなく、変更容易性、障害からの復旧、テスト可能性まで含めて設計したポートフォリオです。辞書データの権利上、リポジトリやデモで公開するデータ範囲を制限する場合があります。
