...
## Stage 0: cutover前提とremote protocol gate

### 対象

- `docs/refactor/local_first/5-migrate-word-status.md`
- `docs/refactor/local_first/6-migrate-my-word.md`
- `docs/refactor/local_first/7-migrate-user-profile.md`
- `lib/features/**/data/sync/*_sync_handler.dart`
- `lib/core/infrastructure/database/firebase/remote_mutation_transaction.dart`
- `firestore.rules`
- dataset別handler test、guest migration test、session test

### 手順

1. Local-first 5〜7の未完了checkboxを現在の実装とtestで再評価する。文書の状態だけを根拠に完了扱いしない。
2. 全handlerで次の共通契約を満たす。
   - server-confirmed transaction成功時だけoutboxをackする。
   - `lastMutationId`再送は成功no-opとして扱い、重複deliveryでrevisionを二重加算しない。
   - remote revision不一致を検出し、retry/競合結果を型付きで返す。
   - remote applyとcheckpoint writeを同一Drift transactionにし、失敗時にcursorを進めない。
   - tombstoneは古いremote updateより優先し、他端末へdeleteを伝播する。
   - 同一timestampのpage境界はdocument IDをtie-breakerにして欠落・重複を起こさない。
3. MyWord/MyWordStatusの親子制約をEmulator上でも確認し、親失敗時は子を送信しない。
4. account切替中に旧epochのhandlerがlocal apply/checkpoint/ackを行わないtestを5 datasetに対して共通化する。
5. 未達の契約が1件でもあればStage 2のproduction cutoverを禁止する。

### Stage gate

- 5 datasetすべてがretry、account分離、競合、delete、重複deliveryのcontractを通る。
- Local-first 5〜7のPhase 8依存項目に未解決のblockerがない。

## Stage 1: registry完全性、trigger、観測性を先に固定

### 対象候補

- `lib/features/sync/application/dataset_handler_registry.dart`
...
- `lib/features/sync/application/model/sync_report.dart`
- 新規: `lib/features/sync/application/sync_trigger_coordinator.dart`（名称は実装時に確定）
- 新規/更新test: `test/unit/features/sync/application/**`
- 新規test: `test/unit/app/bootstrap/sync_composition_test.dart`

### 手順

1. `DatasetHandlerRegistry`へ登録datasetのread-only集合または`validatePlan(DatasetPlan)`を追加する。
2. production composition testで次を固定する。
   - `DatasetPlan.localFirst.datasets.toSet() == SyncDataset.values.toSet()`。
   - registryのdataset集合がplanと完全一致する。
   - duplicateだけでなくmissing/unplanned handlerも起動前に失敗する。
3. productionでは`handler unavailable`を通常skipとして許容しない。optional datasetを将来導入する場合だけ、別の明示的policyを追加する。
4. trigger生成と`SyncContext`生成を1か所へ集約し、すべて同じscheduler/single-flight/session fenceを通す。
   - cold start/sign-in: `AppSessionReady`の`fireImmediately`経路。
   - account切替: 旧tokenをcancelし、新epochがReadyになってから新cycleを起動。
   - resume: `AppLifecycleState.resumed`。
   - network復帰: connectivity adapterはwake signalだけを発行する。
   - local mutation後: Drift transaction commit成功後だけwakeする。rollback時はwakeしない。
   - manual refresh: UIはschedulerを直接組み立てず、共通trigger APIを呼ぶ。
5. 重複triggerは既存single-flight/coalesceへ流し、並列cycleを生成しないtestを追加する。
6. schedulerの戻り値をcutover用observerへ渡す。ログはaccount IDを必要なら不可逆にredactし、dataset、reason、result種別、件数、duration、retryable error codeだけを記録する。token、email、payload、entity本文は記録しない。
7. `SyncReport`のUI接続・telemetry送信は行わず、Phase 2-6が購読できるportだけを維持する。

### Stage gate

- 7種類のforeground triggerがすべて新schedulerへ到達する。
- 同一datasetを処理するproduction runnerが`SyncEngine`以外に存在しないことをtestで示せる。
- registry欠落はapp起動時またはprovider test時に即時失敗する。

## Stage 2: production compositionを新Engine単独へcutover
...
- `lib/features/my_word/di/usecase_di.dart`
- `lib/features/my_word/domain/usecase/my_word/sync_my_word/**`
- `lib/features/my_word/domain/usecase/my_word_status/sync_myword_status/**`
- `test/unit/features/sync/result_propagation_test.dart`
- `test/unit/features/sync/sync_checkpoint_scoping_test.dart`

### 手順

1. `applicationLifecycleEffectsProvider`から`autoSyncProvider`のwatchを削除する。
2. `syncServiceProvider`、`_syncWithRemoteProvider`、`autoSyncProvider`を削除し、`lib/features/sync/di.dart`を削除する。
3. `syncMyWordUseCaseProvider`と`syncMyWordStatusUseCaseProvider`およびそのimportを`my_word/di/usecase_di.dart`から削除する。
4. 旧`SyncMyWordInteractor`、`SyncMyWordStatusUsecase`、`ISyncUseCase`、未使用の`ISyncMyWordUseCase`を削除する。
5. 旧実装だけを検証する`result_propagation_test.dart`と`sync_checkpoint_scoping_test.dart`は単純削除せず、保証していた「部分失敗で他dataset継続」「失敗時cursor不変」「account/dataset scope分離」を新Engine/handler/checkpoint store testへ移してから削除する。
6. `features/sync/description.md`は新Engine構成に書き換えるか、contextsと重複するなら削除する。旧`SyncService`の設計説明は残さない。

### 参照0の確認

```powershell
rg -n "SyncService|ISyncUseCase|syncServiceProvider|autoSyncProvider|SyncMyWordInteractor|SyncMyWordStatusUsecase" lib test
```

期待値は0件である。コメント、test名、互換typedefも残さない。

## Stage 3: app-facing Repositoryをlocal-onlyへ分離

### MyWord / MyWordStatus

対象:

- `lib/features/my_word/domain/i_repository/i_my_word_repository.dart`
- `lib/features/my_word/domain/i_repository/i_my_word_status_repository.dart`
- `lib/features/my_word/data/repository_impl/my_word_repository.dart`
- `lib/features/my_word/data/repository_impl/my_word_status_repository.dart`
- `lib/features/my_word/di/data_di.dart`
- `test/helpers/fake_my_word_repository.dart`
- MyWord/MyWordStatus repository/usecase test

手順:

1. `IMyWordRepository`からremote CRUD、旧sync用local差分API、remote/local changed streamを削除し、通常query/command/watchだけを残す。
2. `IMyWordStatusRepository`も同様に通常update/watchだけへ縮小する。
3. Repository実装からremote data source field、Firebase exception import、DTO変換、旧sync helperを削除する。
4. Repository providerはlocal data source + outbox writerだけを注入する。
5. remote providerはhandler providerからだけ解決できる配置へ分離する。app-facing providerからremote providerへの参照をなくす。
6. fakeとunit testを新しいlocal-only contractへ更新する。

### User Profile / provisioning

対象:

- `lib/features/user/domain/i_repository/i_user_repository.dart`
...
- `lib/features/user/data/data_source/remote/**`

手順:

1. editable profileのread/write（Drift SoT）と、Auth identityに伴うremote provisioning/authority readを別ポートに分ける。
2. 通常profile RepositoryはDrift + outboxだけを持つ。
3. `ensureUserProfile`、account document初期作成、email/subscription等のauthority readは、Auth lifecycleからのみ使えるprovisioning adapterへ移す。
4. provisioning adapterはFirebaseを使ってよいが、editable fieldの通常更新writerにはしない。`username`同期は引き続き`UserProfileSyncHandler`だけが送信する。
5. `UserDTO`からFirestore型変換を分離し、domain/application/通常RepositoryへFirebase SDK型を漏らさない。

### Stage gate

- UI/Applicationから解決できる通常Repositoryにremote CRUD/listener APIがない。
- app-facing RepositoryのconstructorにFirebase/remote data sourceがない。
- local command成功はDrift transaction成功だけを意味し、remote失敗を同期的に返さない。

## Stage 4: 旧remote listener・writerをadapterから削除

### 対象候補

- `lib/features/my_word/data/data_source/remote/myword/**`
- `lib/features/my_word/data/data_source/remote/status/**`
- `lib/core/infrastructure/datasource/word_status/**remote**`
- `lib/core/infrastructure/datasource/jpn_esp_word_status/**remote**`
- `lib/core/infrastructure/database/firebase/daos/**`
- `lib/features/esp_jpn_word_status/data/word_status_entity.dart`
- `lib/features/jpn_esp_word_status/data/jpn_esp_word_status_entity.dart`

### 手順

1. 各handlerが実際に使うremote portを`fetch page/by-id + patch mutation`へ縮小する。
2. `watchChangedIds`、`watchAll`、`.snapshots()`、旧単件writer、batch writer、hard deleteを呼び出し側から順に削除する。
3. remote wakeが必要な場合もFirestore snapshotをlocalへ直接applyせず、共通triggerへsignalするだけの専用adapterとして実装する。初期cutoverではlistener自体を持たず、foreground full/delta pullで収束させる。
4. Firestore `Timestamp`/`DocumentSnapshot`依存はsync remote DTO/mapper内へ閉じ込め、通常data entityをpure Dartにする。
5. word statusのcore配下remote実装は、許可境界が明確になるよう各featureの`data/sync/remote`（または同等の統一パス）へ移す。これは挙動変更と混ぜず、import移動後に同じhandler contract testを通す。

### 参照0の確認

```powershell
rg -n "watchRemoteChangedIds|watchChangedIds|watchAll|\.snapshots\(\)|updateBatchRemote|updateRemote|deleteRemote" lib
```

Firestore listenerが別用途で必要な場合は、該当箇所を許可一覧へ黙って追加せず、Auth authorityまたはsync wake-onlyである根拠とtestをplanへ追記する。

## Stage 5: SharedPreferences checkpointを2リリースで廃止

旧checkpointを読み継ぐとcursor形式の差やaccount scope欠落でデータを取りこぼすため、新Drift checkpointへ値をコピーしない。upgrade後は新checkpoint未作成としてfull pullし、収束後にDrift cursorを保存する。

### Release A: cutover・旧key消去リリース

1. bootstrap時に1回だけ走るversioned cleanupを追加する。
2. `lastSync_wordStatus`と`sync_checkpoint.v1.` prefixの全keyを列挙して削除する。
3. cleanup成功を示すversion markerだけを端末設定として保存する。旧cursor値は読まない。
4. cleanup失敗時は新Engine起動を止めず、旧値を無視してfull syncする。次回起動でcleanupを再試行する。
5. upgrade testで旧keyが消え、Drift checkpointが空のため5 datasetがfull pullされ、成功後だけ新cursorが保存されることを確認する。

### Release B: cleanup code除去リリース

Release Aが最低1つのサポート対象リリースとして配布され、upgrade telemetryまたは受入確認で旧key消去が確認できた後に行う。

- `SharedPreferencesSyncStatusDao`
- `SharedPreferencesSyncStatusDataSource`
- `ISyncStatusDataSource`
- `SyncStatusRepository`
- `ISyncStatusRepository`
- 旧`SyncCheckpoint`/`SyncCheckpointKey`
- `sharedPreferenceSyncStatusDaoProvider`
- `sharedPreferencesSyncStatusDataSourceProvider`
- `syncStatusRepositoryProvider`
- 対応する旧unit test
- Release Aのversioned cleanup（サポートする直接upgrade元に応じて保持期間を判断）

Release B完了時の確認:

```powershell
rg -n "SharedPreferencesSyncStatus|ISyncStatus|SyncStatusRepository|SyncCheckpointKey|lastSync_wordStatus|sync_checkpoint\.v1" lib test
```

期待値は0件である。端末固有設定用のSharedPreferences自体は削除対象ではない。

## Stage 6: Firebase import allowlistとlegacy禁止rule

### 対象

- `tool/import_boundaries/rules.json`
- `tool/check_import_boundaries.dart`
- `test/tool/import_boundaries/check_import_boundaries_test.dart`
- `test/unit/features/sync/application/sync_import_boundary_test.dart`
- `tool/import_boundaries/baseline.json`

### 許可する境界

- `lib/features/auth/data/**`のFirebase Auth infrastructure。
- `lib/app/bootstrap/**`のFirebase初期化。
- 各featureで明示した`data/sync/remote/**`（または採用した統一remote adapter path）。
- Emulator integration test。

### 禁止する境界

- `domain/**`、`presentation/**`、通常`application/**`。
- app-facing `repository_impl/**`とlocal data source/DAO。
- `features/sync/application/**`および`features/sync`のorchestration本体。
- 上記allowlist外のFirebase/Firestore import。
- 旧sync symbol/fileへのimport。

### 手順

1. 現行のpath globの抜け（例: `repository_impl`）をfixture testで再現してからruleを修正する。
2. forbidden directory列挙だけでなく、Firebase package importに対するallowed source path方式をcheckerへ追加する。
3. `firebase_core`、`firebase_auth`、`cloud_firestore`を同じallowlistで検査する。
4. Phase 8で追加するruleにはbaselineを作らない。違反0でなければCIを失敗させる。
5. 既存baselineのうちPhase 8の移動で自然に消えた項目だけを削除し、無関係なarchitecture debtはこのフェーズで修正しない。
...
## Stage 7 実施結果（2026-08-07）

- `firebase-tests/sync_rules_test.mjs` を追加した。production の
  `firestore.rules` を直接読み込み、Auth/Firestore Emulator 上で5 dataset
  の owner write、匿名・別 account の拒否、許可外 field・権限 field・hard
  delete の拒否、transaction revision、duplicate mutation の no-write 分岐、
  server timestamp の確定を検証する。
- `.github/workflows/quality.yml` に通常の Flutter suite と分離した
  `firestore-emulator` job を追加した。この job は Java 21、Node 20、
  `auth,firestore` Emulator を使用し、上記 harness を実行する。CI 用の
  `firebase-tests/firebase.json` は git ignore される開発用 `firebase.json`
  に依存しない。
- `flutter analyze` は成功し、`flutter test` は 287 tests passed、
  import-boundary check は baseline check 成功だった。Dart format check は
  既存の対象外 63 ファイルの未整形により失敗した（本 Stage では整形しない）。
- ローカルの Firebase CLI は JDK 8 では現行 Emulator を起動できないため、
  Emulator harness の実行結果は未取得である。Java 21 以上を導入して README
  記載のコマンドを実行するか、上記 CI job の成功を確認するまで、Emulator
  acceptance と Local-first 8 全体の完了は宣言しない。
