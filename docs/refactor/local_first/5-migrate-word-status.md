...
- [ ] statusのread/writeがDriftだけを通る
- [ ] 通常status RepositoryにFirebase操作がない
- [ ] 両directionがSyncEngineへ登録されている
- [ ] 旧status listenerと旧sync UseCaseがdataset registryから外れている
- [ ] failure、retry、conflict、account切替testが通る
