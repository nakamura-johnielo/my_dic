...
この文書は、Phase 1-1〜1-3のリファクタ中に作られたが、現時点ではまだ本格利用していない、または後続フェーズで育てる前提のファイルを見失わないためのメモである。

Phase 1-3までの主目的は、composition root、import境界、route contractの入口を作ることだった。そのため、Local-first移行、CurrentSession導入、feature ownership整理、旧sync削除までは意図的に踏み込んでいない足場が残っている。

## 読み方

- 「今の状態」は、Phase 1-3完了時点でどこまで接続されているかを示す。
