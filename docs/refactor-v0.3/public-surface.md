...
- shared `Result<T>`
- identity: `CatalogId`, `CatalogWordRef`, `CatalogRankingEntryRef`
...
- ranked-entry feed query/result and semantic entry detail/content nodes
...
- `port/composition.dart`: framework-free typed application composition factory

It is not a business API and is not exported by `catalog.dart`.
